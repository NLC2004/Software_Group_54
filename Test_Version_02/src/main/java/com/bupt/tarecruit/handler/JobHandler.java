...
public class JobHandler extends BaseHandler {
    ...
    private void listJobs(HttpExchange ex) throws IOException {
        ...
        String department = getQueryParam(ex, "department");
        String publishDate = getQueryParam(ex, "publishDate");
        ...
        if (status != null && !status.isEmpty()) {
            ...
            if ("HIRING".equals(st)) st = "OPEN";
            else if ("CLOSED".equals(st)) st = "CLOSED";
            final String statusNormalized = st;
            jobs = jobs.stream().filter(j -> statusNormalized.equalsIgnoreCase(String.valueOf(j.status))).collect(Collectors.toList());
        }
        ...
        if (department != null && !department.isEmpty()) {
            String dep = department.toLowerCase(Locale.ROOT);
            jobs = jobs.stream().filter(j ->
                    (j.courseName != null && j.courseName.toLowerCase(Locale.ROOT).contains(dep))
                    || (j.title != null && j.title.toLowerCase(Locale.ROOT).contains(dep))
            ).collect(Collectors.toList());
        }
        if (publishDate != null && !publishDate.isEmpty()) {
            jobs = jobs.stream().filter(j -> {
                java.time.LocalDate d = java.time.Instant.ofEpochMilli(j.createdAt)
                        .atZone(java.time.ZoneId.systemDefault()).toLocalDate();
                return publishDate.equals(d.toString());
            }).collect(Collectors.toList());
        }
        ...
        for (Job j : jobs) {
            ...
            List<Application> jobApps = ds.getApplicationsByJob(j.id);
            m.put("applicationCount", jobApps.size());
            m.put("pendingCount", jobApps.stream().filter(a -> "PENDING".equals(a.status)).count());
            m.put("approvedCount", jobApps.stream().filter(a -> "APPROVED".equals(a.status)).count());
            m.put("rejectedCount", jobApps.stream().filter(a -> "REJECTED".equals(a.status)).count());
            m.put("withdrawnCount", jobApps.stream().filter(a -> "WITHDRAWN".equals(a.status)).count());
            result.add(m);
        }
    ...
    private void getJob(HttpExchange ex, String jobId) throws IOException {
        ...
        m.put("posterName", poster != null ? (poster.fullName != null && !poster.fullName.isEmpty() ? poster.fullName : poster.username) : "Unknown");
        List<Application> jobApps = ds.getApplicationsByJob(job.id);
        m.put("applicationCount", jobApps.size());
        m.put("pendingCount", jobApps.stream().filter(a -> "PENDING".equals(a.status)).count());
        m.put("approvedCount", jobApps.stream().filter(a -> "APPROVED".equals(a.status)).count());
        m.put("rejectedCount", jobApps.stream().filter(a -> "REJECTED".equals(a.status)).count());
    ...
    private void applyForJob(HttpExchange ex, String jobId) throws IOException {
        ...
        if (settings.containsKey("recruitmentStart")) {
            try {
                long s = Long.parseLong(settings.get("recruitmentStart"));
                if (s > 0 && now < s) {
                    sendError(ex, 400, "Application period has not started yet");
                    return;
                }
            } catch (NumberFormatException ignored) {}
        }
        if (settings.containsKey("recruitmentEnd")) {
            try {
                long e = Long.parseLong(settings.get("recruitmentEnd"));
                if (e > 0 && now > e) {
                    sendError(ex, 400, "Application period has ended");
                    return;
                }
            } catch (NumberFormatException ignored) {}
        }
